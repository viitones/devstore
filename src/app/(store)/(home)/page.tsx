...
export default async function Home() {
  const [highlightProducts, ...otherProducts] = await getFeaturedProducts()

  return (
    <div className="grid max-h-[860px] grid-cols-9 grid-rows-6 gap-6">
      <Link
        href={`/product/${highlightProducts.slug}`}
        className="group relative col-span-6 row-span-6 bg-zinc-900 overflow-hidden flex justify-center items-start"
      >
        <Image
          src={highlightProducts.image}
          alt=""
          width={920}
          height={920}
          quality={100}
        ...
        <div className="absolute bottom-28 right-28 h-12 flex items-center gap-2 max-w-[280px] rounded-full border-2 border-zinc-500 bg-black/60 p-1 pl-5">
          <span className="text-sm truncate">{highlightProducts.title}</span>
